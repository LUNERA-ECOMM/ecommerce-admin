...
export async function getServerSideInfo() {
  try {
    ...
    // Map Firestore field names to our expected format
    // Using actual field names from Firestore: name, slogan, heroDescr, categoryDesc
    return {
      companyName: data.name || '',
      companyTagline: data.slogan || '',
      heroMainHeading: data.heroMainHeading || '',
      heroDescription: data.heroDescr || '',
      categorySectionHeading: data.categorySectionHeading || '',
      categorySectionDescription: data.categoryDesc || '',
