  ...
  return new Promise((resolve) => {
    ...
    const unsubscribe = subscribeToAuth(async (user) => {
      ...
      try {
        ...
        const eventRef = doc(collection(db, ...getCollectionPath('userEvents')));
...
const updateEntityMetrics = async (eventType, entityId, db) => {
  try {
    if (eventType === 'view_category') {
      const categoryRef = doc(db, ...getDocumentPath('categories', entityId));
      ...
      const productRef = doc(db, ...getDocumentPath('products', entityId));
      await runTransaction(db, async (transaction) => {
